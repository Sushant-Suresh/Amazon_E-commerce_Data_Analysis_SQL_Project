...
products > customers > sellers > orders > returns
*/
...
-- Q1. What Is the Total Sale and Average Sale for Goa, Uttarakhand & Bihar (Round-Off to 2 Decimal Places)?
SELECT state,
...
-- Q2. What Is the Total Revenue Generated by Each State?
SELECT state,
...
-- Q3. How Many Orders Were Placed by Each Customer, and What Is Their Average Order Quantity?
SELECT customer_id,
...
-- Q4. Which Category Has the Second Highest Average Sale Amount Per Order?
SELECT category,
       ROUND(AVG(sale)::numeric, 2) AS avg_sale
FROM orders
...
-- Q5. Identify the Top 3 Best-Selling Products (Sub-Categories) in Terms of Total Quantity Sold.
SELECT sub_category,
...
-- Q6. Find Top 3 Products Which Generated Revenue > 10000.
SELECT product_id,
       SUM(sale) AS revenue
FROM orders
GROUP BY product_id
...
-- Q7. Find the Best Selling Month in 2022 Based on Revenue.
...
-- Q8. Find Customer Names Having Total Orders > 100 and Sort Them by Their Total Revenue in Descending Order.
...
-- Q9. Identify All Orders That Have Been Returned, Along With the Details of the Returns (If Available).
SELECT *
FROM orders AS o
RIGHT JOIN returns AS r
ON o.order_id = r.order_id;

-- Q10. Find All the Instances Where Products Have Been Sold, Returned, or Both, Along With the Associated Details.
SELECT p.product_id,
       p.product_name,
...
-- Q11. Find Each Customer’s Latest and Second Latest Order Amount.
...
-- Q12. Identify Top-Selling Products by Revenue for Each Category.
...
-- Q13. Identify Customers With Orders Exceeding the Average Order Value.
SELECT DISTINCT o.customer_id, c.customer_name
FROM orders AS o 
JOIN customers AS c ON o.customer_id = c.customer_id
WHERE o.sale > (SELECT AVG(sale) FROM orders);

-- Q14. Calculate the Return Rate Per Seller.
SELECT s.seller_id,
       s.seller_name,
...
-- Q15. Identify States With Highest Sales in Each Product Category.
SELECT category, 
       state, 
       total_sales
FROM (SELECT o.category,
...
-- Q16. Identify Products With Profit Margin Below Average.
SELECT product_id, 
       product_name, 
	   price, 
	   cogs, 
...
-- Q17. Identify Top 3 Products Frequently Returned Across All Categories.
-- If Multiple Products Have the Same Number of Returns, They Should Be Ranked by the Total Sale Value.
